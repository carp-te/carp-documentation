...
\begin{document} \sloppy
	...
	matrix -threads=8 -krishnaflags="-tmp=./ -threads=2 -log -filtid=0.94 -filtlen=400" chr*.fa
	\end{lstlisting}
	...
	seqer -aligner=muscle -dir=consensus -fasta=true -maxFam=100 -subsample=true -minLen=0.95 -threads=12 -ref=hg19v37.mfa hg94_krishna.igor.gff
	\end{lstlisting}
	...
	* Analysis runs on a machine with 512GB RAM, running Red Hat Linux.
	...
	" "$12; else print $0}' tmp > notKnown.fa.spwb.gff
	\end{lstlisting}
	...
	awk '{if($4>$5) print $1"\t"$2"\t"$3"\t"$5"\t"$4"\t"$6"\t"$7"\t"$8"\t"$9" "$10" "$11
	" "$12; else print $0}' tmp > notKnown.fa.tewb.gff
	...
	awk '{if($4>$5) print $1"\t"$2"\t"$3"\t"$5"\t"$4"\t"$6"\t"$7"\t"$8"\t"$9" "$10" "$11
	" "$12; else print $0}' tmp > notKnown.fa.ervwb.gff
	\end{lstlisting}
