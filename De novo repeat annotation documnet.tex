...
\end{enumerate}
For people that not able to access wu-blast, CENSOR can also use NCBI BLAST instead, please find detail in \href{http://www.girinst.org/downloads/software/censor/} {\color{blue}CENSOR download} page..
\begin{enumerate}
	\item[*] censor.ncbi -lib Vertebrates.fa -lib our\_known\_reps\_20130520.fasta ConsensusSequences.fa 
\end{enumerate}
RepeatMasker can also be used to replace CENSOR in this step (RM-Blast was used as search engine).	
\begin{enumerate}
	\item[*] cat Vertebrates.fa our\_known\_reps\_20130520 > combined\_library.fa
	\item[*] RepeatMasker -pa 16 -a -nolow -norna -dir ./ -lib combine\_library.fa ConsensusSequences.fa
	\item[*] perl format\_RMSK.pl ConsensusSequences.fa > tmp 
	\item[*] mv tmp ConsensusSequences.fa
\end{enumerate}
...
\end{enumerate}
This should generate 5 output files: known.txt, partial.txt, check.txt, notKnown.fa, partial.fa.gff. 
Then we need to further annotate these notKnown.fa consensus sequences. 

\subsubsection{Filter potential protein sequences.}
In this step, you will need to run the perl script reportsJ.sh (perl reportsJ.sh). This uses wu-blast/NCBI-blast blastx, you will also need the uniprot database. reportsJ.sh will ask for a list of databases to process, "libs.txt". This will include notknown.fa and partial.fa.gff, put libs.txt into the current directory. \\

\noindent\textbf{\textcolor{red}{Notes: reportsJ.sh contains three parts: 1) identify uniprot, 2) GB\_TE, 3) retroviruses. You can comment out two parts, and run each part separately in parallel to save time. \\
